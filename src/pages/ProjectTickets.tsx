...
import { Ticket, Engineer, Message, TicketStatus, TicketNotification, DesignRequest, BedroomSubmission } from '../types';
...
export const ProjectTickets: React.FC = () => {
  ...
  const [currentRoleInternal, setCurrentRoleInternal] = useState<'client' | 'engineer' | 'admin'>(() => {
  ...
  // Keep internal role simulation in sync with actual global auth role updates
  useEffect(() => {
    setCurrentRoleInternal(currentUserRole);
  }, [currentUserRole]);

  const currentRole = currentRoleInternal;
  const setCurrentRole = (role: 'client' | 'engineer' | 'admin') => {
    const savedRole = localStorage.getItem('royal_user_role');
    if (role === 'admin' && savedRole !== 'admin') {
      console.warn("Blocked unauthorized change to admin role");
      return;
    }
    if (role === 'engineer' && savedRole !== 'admin' && savedRole !== 'engineer') {
      console.warn("Blocked unauthorized change to engineer role");
      return;
    }
  ...
  // Engineer States
  const [activeEngineerId, setActiveEngineerId] = useState<string | null>(() => {
  ...
  const handleAddEngineer = async (e: React.FormEvent) => {
    ...
    setIsAddingEng(true);
    console.log(`[handleAddEngineer] Initiating engineer creation form submit. Email: ${engEmail.trim()}`);
    try {
      await addEngineer({
        name: engName.trim(),
        email: engEmail.toLowerCase().trim(),
        specialty: engSpecialty.trim(),
        phone: engPhone.trim()
      });
      console.log(`[handleAddEngineer] Engineer added successfully. Clearing inputs...`);
      setEngName('');
      setEngEmail('');
      setEngSpecialty('');
      setEngPhone('');
      alert('تم تسجيل المهندس واعتماده في النظام بنجاح!');
    } catch (err: any) {
      console.error(`[handleAddEngineer] Failed to add engineer:`, err);
      alert(err?.message || 'حدث خطأ أثناء إضافة المهندس.');
    } finally {
  ...
  const handleClientSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let dReq = designRequests.find(r => 
        r.id.toUpperCase() === q || 
        (r.requestNumber && r.requestNumber.toUpperCase() === q) || 
        r.phone === targetId
      );
      
      let bSub = bedroomSubmissions.find(s => 
        s.id.toUpperCase() === q || 
        (s.requestNumber && s.requestNumber.toUpperCase() === q) || 
        s.clientPhone === targetId
      );

      // If not found locally, query Firestore directly to support cross-device searches
      if (!dReq && !bSub && isFirebaseConnected) {
        try {
          const db = getDb();
          const currentClientId = localStorage.getItem('royal_client_uid') || undefined;
          
          // Query Design Requests
          const drSnap1 = await getDocs(query(collection(db, 'designRequests'), where('requestNumber', '==', q)));
          const drSnap2 = await getDocs(query(collection(db, 'designRequests'), where('phone', '==', targetId)));
          let foundDr = drSnap1.docs[0]?.data() as DesignRequest || drSnap2.docs[0]?.data() as DesignRequest;
          if (!foundDr && targetId.length > 5) {
            const drDoc = await getDoc(doc(db, 'designRequests', targetId));
          ...
          if (foundDr) {
            ...
            // Query Bedroom Submissions
            const bsSnap1 = await getDocs(query(collection(db, 'bedroomSubmissions'), where('requestNumber', '==', q)));
            const bsSnap2 = await getDocs(query(collection(db, 'bedroomSubmissions'), where('clientPhone', '==', targetId)));
            let foundBs = bsSnap1.docs[0]?.data() as BedroomSubmission || bsSnap2.docs[0]?.data() as BedroomSubmission;
            if (!foundBs && targetId.length > 5) {
              const bsDoc = await getDoc(doc(db, 'bedroomSubmissions', targetId));
          ...
          console.error("Direct Firestore request search failed:", dbErr);
        }
      }
      ...
      if (matched) {
        ...
        if (ticket) {
          setSearchedRequest(prev => prev ? { ...prev, ticketId: ticket.id } : null);
  ...
  return (
    <div className="min-h-screen bg-[#faf9f6] py-8 sm:py-12 px-4 sm:px-6 lg:px-8" dir="rtl">
      <div className="max-w-7xl mx-auto">
        ...
        {localStorage.getItem('royal_user_role') === 'admin' && (
